...
! function() {
    function FileBufferReader() {
        ...
        fbr.chunks = {}, fbr.users = {}, fbr.readAsArrayBuffer = function(file, callback, extra) {
            var options = {
                file: file,
            ...
            file.extra && Object.keys(file.extra).length && Object.keys(file.extra).forEach(function(key) {
            ...
            var currentPosition;
            "undefined" != typeof fileUUID.currentPosition && (currentPosition = fileUUID.currentPosition, fileUUID = fileUUID.uuid);
            ...
            if (allFileChunks) {
                "undefined" != typeof userid ? (fbr.users[userid + ""] || (fbr.users[userid + ""] = {
                    ...
                    userid: userid,
                    currentPosition: -1
                }), "undefined" != typeof currentPosition && (fbr.users[userid + ""].currentPosition = currentPosition), fbr.users[userid + ""].currentPosition++, currentPosition = fbr.users[userid + ""].currentPosition) : ("undefined" != typeof currentPosition && (fbr.chunks[fileUUID].currentPosition = currentPosition), fbr.chunks[fileUUID].currentPosition++, currentPosition = fbr.chunks[fileUUID].currentPosition);
                var nextChunk = allFileChunks[currentPosition];
                if (!nextChunk) return delete fbr.chunks[fileUUID], void fbr.convertToArrayBuffer({
                    chunkMissing: !0,
                    currentPosition: currentPosition,
                    uuid: fileUUID
                }, callback);
                nextChunk = fbrClone(nextChunk), "undefined" != typeof userid && (nextChunk.remoteUserId = userid + ""), nextChunk.start && fbr.onBegin(nextChunk), nextChunk.end && fbr.onEnd(nextChunk), fbr.onProgress(nextChunk), fbr.convertToArrayBuffer(nextChunk, function(buffer) {
        ...
        fbr.addChunk = function(chunk, callback) {
            chunk && fbReceiver.receive(chunk, function(chunk) {
            ...
            delete fbReceiver.chunks[chunk.uuid], delete fbReceiver.chunksWaiters[chunk.uuid]
    ...
    function FileBufferReaderHelper() {
        function fileReaderWrapper(options, callback) {
            ...
            file.uuid || (file.uuid = (100 * Math.random()).toString().replace(/\./g, ""));
            var chunkSize = options.chunkSize || 15e3;
            options.extra && options.extra.chunkSize && (chunkSize = options.extra.chunkSize);
            var sliceId = 0,
                cacheSize = chunkSize,
            ...
            reader.onloadend = function(evt) {
                evt.target.readyState == FileReader.DONE && addChunks(file.name, evt.target.result, function() {
                    sliceId++, (sliceId + 1) * sliceSize < file.size ? (blob = file.slice(sliceId * sliceSize, (sliceId + 1) * sliceSize), reader.readAsArrayBuffer(blob)) : sliceId * sliceSize < file.size ? (blob = file.slice(sliceId * sliceSize, file.size), reader.readAsArrayBuffer(blob)) : (file.url = URL.createObjectURL(file), callback({
                        currentPosition: currentPosition,
                        uuid: file.uuid,
                        maxChunks: maxChunks,
                        size: file.size,
        ...
        fbrHelper.readAsArrayBuffer = function(fbr, options) {
            function processChunk(chunk) {
                fbr.chunks[chunk.uuid] || (fbr.chunks[chunk.uuid] = {
                    ...
                    userid: 0
                }, chunk.userid = options.userid || options.extra.userid || 0, chunk.extra = options.extra, fbr.chunks[chunk.uuid][chunk.currentPosition] = chunk, chunk.end && earlyCallback && (earlyCallback(chunk.uuid), earlyCallback = null), chunk.maxChunks > 200 && 200 == chunk.currentPosition && earlyCallback && (earlyCallback(chunk.uuid), earlyCallback = null)
            }
    ...
    function FileSelector() {
        function selectFile(callback, multiple, directory) {
            ...
            file.type = "file", multiple && (file.multiple = !0), directory && (file.webkitdirectory = !0), file.accept = selector.accept, file.onclick = function() {
                file.clickStarted = !0
            }, document.body.onfocus = function() {
                setTimeout(function() {
                    file.clickStarted && (file.clickStarted = !1, file.value || noFileSelectedCallback())
                }, 500)
            }, file.onchange = function() {
                if (multiple) {
                    if (!file.files.length) return void console.error("No file selected.");
                    var arr = [];
                    return Array.from(file.files).forEach(function(file) {
                        file.url = file.webkitRelativePath, arr.push(file)
                    }), void callback(arr)
                }
                return file.files[0] ? (callback(file.files[0]), void file.parentNode.removeChild(file)) : void console.error("No file selected.")
            }, file.style.display = "none", (document.body || document.documentElement).appendChild(file), fireClickEvent(file)
        }

        function fireClickEvent(element) {
            if ("function" == typeof element.click) return void element.click();
            if ("function" == typeof element.change) return void element.change();
            if ("undefined" != typeof document.createEvent("Event")) {
                var event = document.createEvent("Event");
                if ("function" == typeof event.initEvent && "function" == typeof element.dispatchEvent) return event.initEvent("click", !0, !0), void element.dispatchEvent(event)
            }
        ...
        selector.selectSingleFile = function(callback, failure) {
            failure && (noFileSelectedCallback = failure), selectFile(callback)
        }, selector.selectMultipleFiles = function(callback, failure) {
            failure && (noFileSelectedCallback = failure), selectFile(callback, !0)
        }, selector.selectDirectory = function(callback, failure) {
            failure && (noFileSelectedCallback = failure), selectFile(callback, !0, !0)
        }, selector.accept = "*.*"
    }
    ...
    function FileBufferReceiver(fbr) {
        function receive(chunk, callback) {
            ...
            if (chunk.start && !fbReceiver.chunks[chunk.uuid] && (fbReceiver.chunks[chunk.uuid] = {}, fbr.onBegin && fbr.onBegin(chunk)), !chunk.end && chunk.buffer && (fbReceiver.chunks[chunk.uuid][chunk.currentPosition] = chunk.buffer), chunk.end) {
                ...
                blob = merge(blob, chunk), blob.url = URL.createObjectURL(blob), blob.uuid = chunk.uuid, blob.size || console.error("Something went wrong. Blob Size is 0."), fbr.onEnd && fbr.onEnd(blob), delete fbReceiver.chunks[chunk.uuid], delete fbReceiver.chunksWaiters[chunk.uuid]
            }
            chunk.buffer && fbr.onProgress && fbr.onProgress(chunk), chunk.end || (callback(chunk), fbReceiver.chunksWaiters[chunk.uuid] = function() {
                function looper() {
                    chunk.buffer && fbReceiver.chunks[chunk.uuid] && (chunk.currentPosition == chunk.maxChunks || fbReceiver.chunks[chunk.uuid][chunk.currentPosition] || (callback(chunk), setTimeout(looper, 5e3)))
                }
                setTimeout(looper, 5e3)
            }, fbReceiver.chunksWaiters[chunk.uuid]())
        }
        var fbReceiver = this;
        fbReceiver.chunks = {}, fbReceiver.chunksWaiters = {}, fbReceiver.receive = receive
    }

    function merge(mergein, mergeto) {
        if (mergein || (mergein = {}), !mergeto) return mergein;
        for (var item in mergeto) try {
    ...
    var Length = [null, null, "Uint16", "Float64", "Uint8", null, null, "Int8", "Int16", "Int32", "Uint8", "Uint16", "Uint32", "Float32", "Float64", "Uint8", "Uint8", "Uint8"],
        binary_dump = function(view, start, length) {
            var table = [],
                ...
                ROW_LENGTH = 40;
            table[0] = [];
            for (var i = 0; i < ROW_LENGTH; i++) table[0][i] = i < 10 ? "0" + i.toString(10) : i.toString(10);
            for (i = 0; i < length; i++) {
                var code = view.getUint8(start + i, endianness),
                    index = ~~(i / ROW_LENGTH) + 1;
                "undefined" == typeof table[index] && (table[index] = []), table[index][i % ROW_LENGTH] = code < 16 ? "0" + code.toString(16) : code.toString(16)
            }
        ...
        find_type = function(obj) {
            ...
            if (void 0 === obj) type = Types.UNDEFINED;
                ...
                var const_name = obj.constructor.name,
                    const_name_reflection = obj.constructor.toString().match(/\w+/g)[1];
                if (void 0 !== const_name && void 0 !== Types[const_name.toUpperCase()]) type = Types[const_name.toUpperCase()];
                else if (void 0 !== const_name_reflection && void 0 !== Types[const_name_reflection.toUpperCase()]) type = Types[const_name_reflection.toUpperCase()];
        ...
        pack = function(serialized) {
            ...
            for (i = 0; i < serialized.length; i++) {
                var start = cursor,
                    ...
                    unit = null === type_name ? 0 : window[type_name + "Array"].BYTES_PER_ELEMENT;
                switch (type === Types.BUFFER ? view.setUint8(cursor, Types.BLOB, endianness) : view.setUint8(cursor, type, endianness), cursor += TYPE_LENGTH, debug && console.info("Packing", type, TypeNames[type]), type !== Types.ARRAY && type !== Types.OBJECT || (view.setUint16(cursor, length, endianness), cursor += LENGTH_LENGTH, debug && console.info("Content Length", length)), view.setUint32(cursor, byte_length, endianness), cursor += BYTES_LENGTH, debug && (console.info("Header Size", header_size, "bytes"), console.info("Byte Length", byte_length, "bytes")), type) {
                    ...
                    case Types.UNDEFINED:
                        break;
                    case Types.STRING:
                        for (debug && console.info('Actual Content %c"' + value + '"', "font-weight:bold;"), j = 0; j < length; j++, cursor += unit) view.setUint16(cursor, value.charCodeAt(j), endianness);
                        break;
                    case Types.NUMBER:
                    case Types.BOOLEAN:
        ...
        unpack = function(view, cursor) {
            ...
            switch (type) {
                ...
                case Types.NUMBER:
                    ...
                    break;
                case Types.BOOLEAN:
                    value = 1 === view.getUint8(cursor, endianness), cursor += unit, debug && (console.info('Actual Content %c"' + value.toString() + '"', "font-weight:bold;"), binary_dump(view, start, cursor - start));
                    break;
                case Types.INT8ARRAY:
                case Types.INT16ARRAY:
                case Types.INT32ARRAY:
                ...
                case Types.FLOAT64ARRAY:
                case Types.ARRAYBUFFER:
                    elem = view.buffer.slice(cursor, cursor + byte_length), cursor += byte_length, value = type === Types.ARRAYBUFFER ? elem : new window[type_name + "Array"](elem), debug && binary_dump(view, start, cursor - start);
                    break;
                case Types.BLOB:
                    if (debug && binary_dump(view, start, cursor - start), window.Blob) {
                    ...
                    break;
                case Types.ARRAY:
                    for (debug && binary_dump(view, start, cursor - start), value = [], i = 0; i < length; i++) elem = unpack(view, cursor), cursor = elem.cursor, value.push(elem.value);
                    break;
                case Types.OBJECT:
                    for (debug && binary_dump(view, start, cursor - start), value = {}, i = 0; i < length; i++) {
        ...
        serialize = function(obj, callback) {
            var type, subarray = [],
                ...
                length = 0,
                value = obj;
            switch (type = find_type(obj), unit = void 0 === Length[type] || null === Length[type] ? 0 : window[Length[type] + "Array"].BYTES_PER_ELEMENT, type) {
                case Types.UNDEFINED:
                case Types.NULL:
                    break;
                case Types.NUMBER:
    ...
    window.FileConverter = FileConverter, window.FileSelector = FileSelector, window.FileBufferReader = FileBufferReader
}();